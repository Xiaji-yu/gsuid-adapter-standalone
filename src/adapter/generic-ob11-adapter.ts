...
export class GenericOB11Adapter implements ProtocolAdapter {
  readonly type = 'generic-ob11';
  ...
  handleOpen(_ws: WebSocket): void {
    this.logger.debug('[OB11] Connection opened');
  }
  ...
  handleMessage(_ws: WebSocket, raw: string): MessageEnvelope | MetaEnvelope | null {
    try {
      ...
      this.logger.error('[OB11] Failed to parse message:', err);
      return null;
    }
  }

  handleClose(_ws: WebSocket, code: number, reason: string): void {
    this.logger.debug(`[OB11] Connection closed: ${code} ${reason}`);
  }
