...
export class GScoreService {
    ...
    try {
      ...
      if (replySeg) {
        ...
        if (replyId) {
          ...
          try {
            const replyMsg = await ActionDispatcher.getInstance().call(
              'get_msg',
              { message_id: replyId }
            ) as OB11Message;
        ...
        case 'file':
          if (event.message_type === 'private') {
            ...
            try {
              ...
              const resp = await ActionDispatcher.getInstance().call(
              ...
              if (isJsonFile && pluginState.config.privateJsonBase64Enabled) {
                try {
                  ...
                  if (fileSize > maxBytes) {
                    ...
                    await ActionDispatcher.getInstance().call(
                      'send_msg',
                      {
  ...
  private async fetchForwardItems(forwardId: string, depth: number = 0, seen: Set<string> = new Set()): Promise<GsCoreContent> {
    ...
    seen.add(forwardId);
    try {
      const raw = await ActionDispatcher.getInstance().call(
        'get_forward_msg',
        { id: forwardId }
      );
  ...
  private async handleGsCoreMessage(msgSend: GsCoreMessageSend) {
    ...
    try {
      ...
      if (target_type === 'direct') {
        ...
        const ret = await ActionDispatcher.getInstance().call('send_msg', params);
          ...
          if (Array.isArray(msg.data)) {
            ...
            for (const subMsg of subMessagesRaw) {
              ...
              if (ob11Segments.length > 0) {
                // 构造 node 节点（以机器人自身身份作为发送者）
                result.push({
                  type: 'node',
                  data: {
                    user_id: String(pluginState.selfId || ''),
                    nickname: String(pluginState.selfNickname || ''),
